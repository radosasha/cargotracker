...
class LogsLocalDataSourceImpl(
    ...
    override suspend fun createArchive(logsDirectory: String, selectedFiles: List<LogFile>): String {
        return withContext(Dispatchers.Default) {
            try {
                ...
                }_${now.hour.toString().padStart(2, '0')}${now.minute.toString().padStart(2, '0')}${now.second.toString().padStart(2, '0')}"
                ...
                val filesToArchive = selectedFiles.mapNotNull { logFile ->
                    try {
                        val source = fileAppender.getLogFileSource(logFile.name)
                        FileInfo(name = logFile.name, path = logsDirectory, source)
                    } catch (e: Exception) {
    ...
    override suspend fun getLogsDirectory(): String {
